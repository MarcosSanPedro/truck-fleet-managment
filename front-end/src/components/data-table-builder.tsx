...
import { DataTableConstructor, type DataTableConfig as ConstructorDataTableConfig } from "@/lib/data-constructor/constructor";
...
const DataTableRow = React.memo(DataTableRowComponent) as any;
...
/**
 * Simple data table builder that uses the constructor for auto-detection
 */
export function DataTableBuilder<T extends { id?: string | number }>({
  data,
  config = {},
  ...props
}: {
  data: T[];
  config?: ConstructorDataTableConfig;
} & Omit<DataTableProps<T>, 'data' | 'columns'>) {
  // Create constructor instance
  const constructor = React.useMemo(() => {
    return new DataTableConstructor<T>(data, config);
  }, [data, config]);

  // Auto-generate columns from constructor
  const autoGeneratedColumns = React.useMemo(() => {
...
/**
 * Hook to use the constructor with data table
 */
export function useDataTableConstructor<T extends { id?: string | number }>(
  data: T[],
  config: ConstructorDataTableConfig = {}
) {
  const constructor = React.useMemo(() => {
    return new DataTableConstructor(data, config);
  }, [data, config]);
  ...
  return {
    constructor,
    columns,
    constructorData: constructor.constructorData,
    detectedColumns: constructor.detectedColumns,
  };
}
